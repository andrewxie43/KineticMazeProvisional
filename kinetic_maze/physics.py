...
class KineticMazeMotor:
    ...
    def report_odrive_error(self):
        l.error("self.axis().error: %d", self.axis().error)
        l.error("self.axis().motor.error: %d", self.axis().motor.error)
        l.error("self.axis().encoder.error: %d", self.axis().encoder.error)
    ...
    def set_velocity(self, vel, wait=False):
        if abs(vel) < c.physics.VELOCITY_MIN_CUTOFF:
            ...
            corrected = vel
            l.debug("Setting velocity: %.2f -> %.2f", vel, corrected)
        try:
    ...
    def init_odrive(self):
        l.info("Finding ODrive")
        self.od = odrive.find_any()

        l.info("Calibrating ODrive")
        ...
        l.info("Waiting for oscillation to dissipate")
    ...
    def home(self):
        ...
        with self.axis_context(control_mode=odrive.enums.CTRL_MODE_VELOCITY_CONTROL):
            ...
            l.debug("Found first edge at %d", first_edge)

            l.debug("Finding second edge")
            self.axis().cycle_trigger.last_edge_hit.has_hit = False
            while not self.axis().cycle_trigger.last_edge_hit.has_hit:
                pass
            ...
            l.debug("Found second edge at %d (offset %d)", second_edge, delta)
    ...
    def go_to_angle(self, angle, direction=None,
        ...
        with self.axis_context(control_mode=odrive.enums.CTRL_MODE_POSITION_CONTROL,
            ...
            l.debug("Seeking to %d (currently at %f)", target, self.axis().encoder.pos_estimate)
            ...
            l.debug("Trapezoidal planning finished")
